...
//メーター関係
var MeterStartDeg_Keikyu = 36 * 4; //Start角度 135 36 * 4
var MeterRangeDeg_Keikyu = 36 * 7; //Range角度 270 36 * 7

function changeBackground_Keikyu()
{
    document.getElementById("tab-cab").style.backgroundColor = "#303030"; 
   
}

function drawMeterBackground_Keikyu(inScale) {
    ...
    grad.addColorStop(0.00, "#252525");//"#202020");
    grad.addColorStop(0.82, "#252525");
    grad.addColorStop(0.84, "#111111");
    grad.addColorStop(0.85, "#666060");
    grad.addColorStop(0.87, "#252525");
    grad.addColorStop(0.95, "#252525");
    grad.addColorStop(0.98, "#111111");
    grad.addColorStop(1.00, "#909090");
    cv.fillStyle = grad;
    cv.fill();
    ...
    for (var i = 0; i <= LocMeterMaxSpeed; i++) {
        ...
        var radian = ((MeterRangeDeg_Keikyu / LocMeterMaxSpeed) * i + MeterStartDeg_Keikyu) * Math.PI / 180;
        var radian2 = ((MeterRangeDeg_Keikyu / LocMeterMaxSpeed) * i + MeterStartDeg_Keikyu + 1.5) * Math.PI / 180;
        var radian3 = ((MeterRangeDeg_Keikyu / LocMeterMaxSpeed) * i + MeterStartDeg_Keikyu - 1.5) * Math.PI / 180;
        //var xx = center.x + (rLen - 20) * Math.cos(radian);
        //var yy = center.y + (rLen - 20) * Math.sin(radian);
        var pos_x = cPoint(center, rLen - 5, radian);//外側
        var pos_x2 = cPoint(center, rLen - 28, radian);//内側
        var pos_x3 = cPoint(center, rLen - 16, radian);//外側２
...
function drawMeter_Keikyu(inSpeed, inScale) {
    if (CachedImage == 0) {
    ...
    var inValue = Math.round((inSpeed * MeterRangeDeg_Keikyu) / 1023);
   ...
   switch(ROBOT_CurrentState())
   {
    ...
    // 針(影)描画
    var hRadian = (inValue + MeterStartDeg_Keikyu) * Math.PI / 180;
    var pos_x = cPoint(center, rLen - 45, hRadian);//先端
    var pos_x2 = cPoint(center, rLen - 140, hRadian);//途中の変曲点
    ...
    grad2.addColorStop(0.00, "#cccccc");
    grad2.addColorStop(0.45, "#dddddd");
    grad2.addColorStop(0.5, "#555555");
    grad2.addColorStop(1.00, "#666666");
    cv.fillStyle = grad2;
