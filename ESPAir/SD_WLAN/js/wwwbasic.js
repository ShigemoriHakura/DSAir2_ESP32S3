  ...
  function Interpret(code, canvas, from_tag) {
    ...
    function Factor3() {
      if (tok == '(') {
        ...
        {
          Skip('(');
            var getacc_addr = Expression();
          Skip(')');
           
            //DSair2 process
            return "CommandGetAccStatus(" + getacc_addr + ")";
        }  
        if (name == 'getaccw')
        {
          Skip('(');
          ...
          Skip(')');
            ...
            //DSair2 process
            return "CommandGetAccStatusW(" + getacc_addr + ")";
        }  
        if ((name == 'gets88') || (name == 's88get'))
        {
          Skip('(');
            var gs8_addr = Expression();
          Skip(')');
            //DSair2 process
            //S88 scan
            return "CommandGetS88Data(" + gs8_addr + ")";
        }   
        if ((name == 'gets88w') || (name == 's88getw'))
        {
          Skip('(');
            var gs8w_addr = Expression();
          Skip(')');
            //DSair2 process
            //S88 scan
            return "CommandGetS88Word(" + gs8w_addr + ")";
        }   
        
        if (name == 'getslotaddr')
        {
          Skip('(');
            var slot_addr = Expression();
          Skip(')');
            //DSair2 process
            return "GetSlotAdress(" + slot_addr + ")";
        }

        if (name == 'getslotspd')
        {
          Skip('(');
            var slotspd_addr = Expression();
          Skip(')');
            //DSair2 process
            return "GetSlotSpeed(" + slotspd_addr + ")";
        }

        if (name == 'getslotfnc')
        {
          Skip('(');
            var slotfnc_addr = Expression();
          Skip(',');
            var slotfnc_fncno = Expression();
          Skip(')');
            //DSair2 process
            return "GetSlotFnc(" + slotfnc_addr + "," + slotfnc_fncno + ")";
        }
        ...
        if (name == 'getslotfncw')
        {
          ...
          Skip(')');
            //DSair2 process
            return "GetSlotFncW(" + slotfnc_addr + "," + slotfnc_fncno + ")";
        }
    ...
    function Statement() {
      ...
      {
          Skip('pwr');
          var pwr_prm = Expression();
          //DSair2 process

          curop += "onClickPon(" + pwr_prm + ");\n";
          curop += "Sleep(1000);\n"
          NewOp();
        }
      else if (tok == 'acc')
      {
          Skip('acc');
          var acc_addr = Expression();
          Skip(',');
          var acc_dir = Expression();
          //DSair2 process
          curop += "CommandTurnout(" + acc_addr + "," + acc_dir + ");\n";
          curop += "Sleep(250);\n"
          NewOp();
        }
      else if (tok == 's88start')
      {
          Skip('s88start');
          //DSair2 process
          curop += "CommandGetS88();\n";
      ...
      {
          Skip('spd');
          var spd_addr = Expression();
          Skip(',');
          var spd_speed = Expression();
          //DSair2 process
          curop += "CommandLocSpeed(" + spd_addr + "," + spd_speed + ");\n";
          curop += "Sleep(250);\n"
          NewOp();
        }
        else if (tok == 'dcpwm')
        {
            Skip('dcpwm');
            var dcpwm_speed = Expression();
            //DSair2 process
            curop += "ANA_BASSpeed(" + dcpwm_speed + ");\n";
            curop += "Sleep(250);\n"
            NewOp();
        }
        else if (tok == 'dcdir')
        {
            Skip('dcdir');
            var dcdir_dir = Expression();
            //DSair2 process
            curop += "ANA_Fwd(" + dcdir_dir + ");\n";
            curop += "Sleep(250);\n"
            NewOp();
        }
      else if (tok == 'dir')
      {
          Skip('dir');
          var dir_addr = Expression();
          Skip(',');
          var dir_dir = Expression();
          //DSair2 process
          curop += "CommandLocDirection(" + dir_addr + "," + dir_dir + ");\n";
          curop += "Sleep(250);\n"
          NewOp();
        }
      else if (tok == 'fnc')
      {
          Skip('fnc');
          var fnc_addr = Expression();
          Skip(',');
          var fnc_no = Expression();
          Skip(',');
          var fnc_pwr = Expression();

          //DSair2 process
          curop += "CommandLocFunction(" + fnc_addr + "," + fnc_no + "," + fnc_pwr + ");\n";
          curop += "Sleep(250);\n"
          NewOp();
        }
      else if (tok == 'fnx')
      {
          ...
          //DSair2 process
          //DCCアドレス,ファンクション番号
          curop += "CommandLocFunction(" + fnc_addr + "," + fnc_no + ",1);\n";
          curop += "Sleep(1500);\n"
          curop += "CommandLocFunction(" + fnc_addr + "," + fnc_no + ",0);\n";
          curop += "Sleep(250);\n"
          NewOp();
        }
      else if (tok == 'cvw')
      {
          Skip('cvw');
          var cvw_addr = Expression();
          Skip(',');
          var cvw_val = Expression();
          //DSair2 process
      }
        else if (tok == 'mp4play')
      {
          Skip('mp4play');
          var mp4_file = Expression();
       
          //DSair2 process
          curop += "playVideo(" + mp4_file + ");\n";
          NewOp();
      }
        else if (tok == 'mp3play')
      {
          Skip('mp3play');
          var mp3_file = Expression();
       
          //DSair2 process
          curop += "playSound(" + mp3_file + ");\n";
          NewOp();
      }
      else if (tok == 'mp3stop')
      {
