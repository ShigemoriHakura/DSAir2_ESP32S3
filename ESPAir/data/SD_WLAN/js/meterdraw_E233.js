...
//メーター関係
var MeterStartDeg_E233 = 36 * 4; //Start角度 135 36 * 4
var MeterRangeDeg_E233 = 36 * 7; //Range角度 270 36 * 7

function changeBackground_E233()
{
    //BODYの背景画面色切り替え
    //document.body.style.backgroundColor = "#3B4B6E";  

    //タブの背景を切り替え
    document.getElementById("tab-cab").style.backgroundColor = "#3B4B6E"; 
    //document.getElementsByClassName("ui-tabs-nav").style.backgroundColor = "#3B4B6E"; 
/*
    var elements = document.getElementsByClassName('ui-tabs-nav');
    for(i=0;i<elements.length;i++){
      elements[i].style.backgroundColor = "#3B4B6E";
    }
*/
    
}

function drawMeterBackground_E233(inScale) {

    changeBackground_E233();
    ...
    grad.addColorStop(0.85, "#202020");
    grad.addColorStop(0.86, "#66667A");
    //grad.addColorStop(0.87, "#AAAAAA");
    grad.addColorStop(1.00, "#66667A");
    cv.fillStyle = grad;
    cv.fill();
    ...
    cv.arc( center.x, center.y, rLen - scale / 3.5, ((MeterRangeDeg_E233 / 120) * 0 + MeterStartDeg_E233) * Math.PI / 180, ((MeterRangeDeg_E233 / 120)*120 + MeterStartDeg_E233) * Math.PI / 180, false ) ;
    cv.stroke();

    // Canvasの線の色、塗りつぶし色
    cv.fillStyle = "#3B4B6E";
    cv.strokeStyle = "#3B4B6E";//"#EEEEEE";

    cv.beginPath();
    // 線の太さ
    cv.lineWidth = 2 ;
    cv.moveTo(center.x,center.y)
    cv.arc( center.x, center.y, rLen, ((MeterRangeDeg_E233 / 120) * 120 + MeterStartDeg_E233) * Math.PI / 180, ((MeterRangeDeg_E233 / 120)*0 + MeterStartDeg_E233) * Math.PI / 180, false ) ;
    cv.fill();
    cv.stroke();
    /*
    cv.beginPath();
    cv.arc( center.x, center.y, rLen - 37, ((MeterRangeDeg_E233 / 120) * 0 + MeterStartDeg_E233) * Math.PI / 180, ((MeterRangeDeg_E233 / 120)*120 + MeterStartDeg_E233) * Math.PI / 180, false ) ;
    cv.stroke();
    */

    // メモリ表示
    for (var i = 0; i <= LocMeterMaxSpeed; i++) {
        var radian = ((MeterRangeDeg_E233 / LocMeterMaxSpeed) * i + MeterStartDeg_E233) * Math.PI / 180;
        var radian2 = ((MeterRangeDeg_E233 / LocMeterMaxSpeed) * i + MeterStartDeg_E233 + 1.5) * Math.PI / 180;
        var radian3 = ((MeterRangeDeg_E233 / LocMeterMaxSpeed) * i + MeterStartDeg_E233 - 1.5) * Math.PI / 180;
        ...
        var pos_x4 = cPoint(center, rLen - scale / 3.5, radian);//内側
        var pos_x5 = cPoint(center, rLen - 30, radian);//内側
        var aMemSize = 1;
    ...
    cv.fillText("km/h", center.x - (aMetrics2.width / 2) + scale /4, center.y + scale / 2.0 - 8);
...
function drawMeter_E233(inSpeed, inScale) {
    if (CachedImage == 0) {
    ...
    var inValue = Math.round((inSpeed * MeterRangeDeg_E233) / 1023);
   ...
   switch(ROBOT_CurrentState())
   {
    ...
    cv.fillText(Math.round(inSpeed * LocMeterMaxSpeed / 1024), center.x - (aMetrics.width) + scale / 7, center.y +
        scale / 2.05);
    ...
    // 針(影)描画
    var hRadian = (inValue + MeterStartDeg_E233) * Math.PI / 180;
    var pos_x = cPoint(center, rLen - 30, hRadian);//先端
    var pos_x2 = cPoint(center, rLen - 70, hRadian);//途中の変曲点
    var a90deg = Math.PI / 2;
    var pos_x3 = cPoint(center, rLen - 170, hRadian + a90deg * 2);//逆側の変曲点
    cv.strokeStyle = "#EEEEEE";
    cv.fillStyle = "#EEEEEE";
    cv.lineWidth = scale / 100;
    cv.beginPath();
    cv.moveTo(pos_x3.x + 9 * Math.cos(hRadian - a90deg), pos_x3.y + 9 * Math.sin(hRadian - a90deg));
    cv.lineTo(pos_x2.x + 9 * Math.cos(hRadian - a90deg), pos_x2.y + 9 * Math.sin(hRadian - a90deg));
    cv.lineTo(pos_x.x, pos_x.y);
    cv.lineTo(pos_x2.x + 9 * Math.cos(hRadian + a90deg), pos_x2.y + 9 * Math.sin(hRadian + a90deg));
    cv.lineTo(pos_x3.x + 9 * Math.cos(hRadian + a90deg), pos_x3.y + 9 * Math.sin(hRadian + a90deg));
    cv.closePath();
    //cv.stroke();
    cv.fill();
    // 針(中心)描画
